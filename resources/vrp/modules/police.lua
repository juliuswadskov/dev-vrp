...
local choice_drag = {function(player,choice)
  vRPclient.getNearestPlayer(player,{10}, function(nplayer)
    if nplayer then
      local nuser_id = vRP.getUserId(nplayer)
      if nuser_id then
        vRPclient.getFollowedPlayer(nplayer, {}, function(followed) 
          if followed ~= player then -- drag
            vRPclient.followPlayer(nplayer, {player})
          else -- stop follow
            vRPclient.followPlayer(nplayer)
          end
        end)
      else
        vRPclient.notify(player,{lang.common.no_player_near()})
  ...
  vRPclient.getNearestPlayer(player,{10}, function(nplayer)
    ...
    if nuser_id then
      vRPclient.isHandcuffed(nplayer, {}, function(isHandcuffed)
        if isHandcuffed then  -- check handcuffed
                ...
                if amount > 0 then
                  ...
                  if item_name then -- do transfer
                    if vRP.tryGetInventoryItem(nuser_id,idname,amount,true) then
                      ...
                      vRPclient.notify(player,{lang.police.menu.seize.seized({item_name,amount})})
                    end
                  end
                end
              end
...
local choice_jail = {function(player, choice)
  ...
  if user_id then
    vRPclient.getNearestPlayer(player, {5}, function(nplayer) 
      local nuser_id = vRP.getUserId(nplayer)
      if nuser_id then
        vRPclient.isJailed(nplayer, {}, function()
          if isJailed then
