...
local function ch_audiosource(player, choice)
  ...
  if name and string.len(name) > 0 then
    if url and string.len(url) > 0 then
      vRPclient.getPosition(player, {}, function(x,y,z)
        vRPclient.setAudioSource(-1,{"vRP:admin:"..name,url,0.5,x,y,z,125})
      end)
    else
      vRPclient.removeAudioSource(-1,{"vRP:admin:"..name})
    end
  end
end
...
RegisterCommand("revive", function(source)
  ...
  if vRP.hasPermission(user_id, "superadmin.revive") then
    vRP.setHunger(user_id, 0)
    vRP.setThirst(user_id, 0)
    vRPclient.setHealth(source, {200})
  else
      vRPclient.notify(source, {"Du har ikke adgang til denne command"})
  end
end, false)
    ...
    choices["Admin"] = {function(player,choice)
      vRP.buildMenu("admin", {player = player}, function(menu)
        ...
        menu.onclose = function(player)
          vRP.buildMenu("main", {player = player}, function(menudata)
            menudata.name = "Menu"
            menudata.css = {top="75px", header_color="rgba(100, 0, 0,0.75)"}
            vRP.openMenu(player,menudata) -- open the generated menu
          end)
        end -- nest menu
